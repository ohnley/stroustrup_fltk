...
Box::Box(int xc, int yc, int ww, int hh, int round)
    :r(round), x(xc), y(yc), w(ww), h(hh)
{
    add(Point{x, y});
...
{
    fl_line(point(0).x + r, point(0).y, point(0).x + w - r - 1, point(0).y);
    fl_line(point(0).x + r, point(0).y + h, point(0).x + w - r - 1, point(0).y + h);
